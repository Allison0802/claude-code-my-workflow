...
Run the mandatory proofreading protocol on manuscripts and documents. This produces a report of all issues found WITHOUT editing any source files.

## Steps

1. **Identify files to review:**
   - If `$ARGUMENTS` is a specific filename: review that file only
   - If `$ARGUMENTS` is "all": review all source files in `Papers/*.tex`, `Slides/`, and `Quarto/`
